...
def cross_entropy_loss(
    ...
    # takes in predicted logits (oi) and targets (xi+1)
    # computes the cross entropy ℓi = − log softmax(oi )[xi+1]
    lse = torch.logsumexp(inputs, dim=-1)
    o_t = inputs.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)
    return (lse - o_t).mean()
